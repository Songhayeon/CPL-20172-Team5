...
and the Secure MovieSocket server on port $MOVIE_PORT.
...
MOVIE_LOG='/tmp/movie.websocket.log'
...
./demos/web/moviesocket-server.py --port $MOVIE_PORT 2>&1 | tee $MOVIE_LOG &
wait
